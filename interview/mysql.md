1. 基础知识
   ...
   7. 一条sql语句的执行过程
      ...
      1. 连接器：负责跟客户端建立连接、获取权限、管理连接。用户登录成功后，会从权限表查出此用户的权限
      2. 分析器
      3. 优化器
      4. 执行器：执行前判断有没有权限
...
13. 使用长连接后，mysql内存增长过快，有可能会被linux kill

    1. 定期断开长连接，或者执行大的操作直接断开连接，下次查询再重新连接
    2. 执行大的操作后，可以执行mysql_reset_connection来冲洗初始化连接资源
