...
7. kafka
   ...
   7. kafka的消费策略
      ...
      1. range：以topic为单位进行顺序分配
      2. roundRobin：以所有topic下的所有分区进行均匀分配
      3. stick：尽可能保留现有分配

   8. 分区器、序列化器、拦截器是什么？执行顺序？

      1. 拦截器->序列化器->分区器

   9. kafkaProducer的整体结构？

   10. kafkaProducer有几个线程？分别做什么工作？
   ...
   11. “消费组中的消费者个数如果超过topic的分区，那么就会有消费者消费不到数据”这句话是否正确？如果正确，那么有没有什么hack的手段？
   ...
   22. kafkaConsumer是非线程安全的，怎么样实现多线程消费？
   ...
   24. Kafka有哪几处地方有分区分配的概念？简述大致的过程及原理？
   ...
   31. kafka有哪几处地方有分区分配的概念？简述大致过程和原理
   ...
   41. Group Coordinator和Consumer Coordinator的关系
...
14. 锁
       ...
       3. 示例图（ix：表级意向写锁）：![image-20211118131842460](image-20211118131842460.png)
       4. 间隙锁是什么？什么情况会加间隙锁？
       5. mysql的语句加上next lock，sql应该怎么写
       6. mysql的语句具体在什么时候加锁？
...
10. redis
   ...
   2. 5种基本数据结构+bitmap、geo、hyperLoglog+redis module+redis stream
