...
2. 对象创建过程？
   ...
   > 2. 如果不能在栈上分配，会尝试在TLAB(thread local allocation buffer：默认大小是eden区的1%)上分配。（Tlab是为了加快对象分配速度，防止每次分配对象都需要cas，每个TLAB只能由一个线程创建对象，但是创建出来的对象是共享的）
   > 3. 如果TLAB分配失败，如果内存是规整的就进行指针碰撞，否则就在空闲列表找一块内存分配。

3. 什么情况发生OOm异常

   > 1. 堆内存溢出：内存泄漏、内存不够
   > 2. 方法区溢出：动态创建了大量的类, 类加载器重复加载
   > 3. 堆外内存：容易内存泄漏
...
21. 类加载过程？类加载方式？tomcat如何破坏双亲委派模型？
    ...
    > 3. 不打破双亲委派模型就重写findClass方法
    > 3. 打破双亲委派模型就重写loadClass方法

22. java的类不能在类加载器中共享， 对象是可以共享的
...
11. 计网
    ...
    26. 浏览器输入一个域名发生了什么？
        ...
        5. nginx->web
        6. 解析请求
        6. 示例图：![image-20220502101100946](image-20220502101100946.png)
...
11. kafka
    ...
    4. ISR、ar、hw、leo、lso、lw？
       ...
       8. 实例图：![image-20230413221109194](image-20230413221109194.png)
...
11. mysql
    ...
    4. mysql在执行过程中临时使用的内存是管理在连接对象里面， 这些资源会在连接断开时才释放， 如果长连接过多，可能导致内存占用过大造成OOM。从现象上来看就是mysql异常重启。 解决方案

       1. 定期断开长连接。在使用一段时间或执行一个占用内存过大的查询后，断开重连
       2. 如果是mysql5.7及更新的版本， 可以使用mysql_reset_connection来初始化资源
       
    5. 数据库备份有的是一天一备，有的是一周一备。在什么场景下一天一备比一周一备更有优势？
    ...
    6. 为什么要WAL？
       ...
       1. 性能： 由随机写变成顺序写
       2. 日志相当于是对数据的一种特殊备份，可以在崩溃时恢复

    7. 为什么binlog无法实现crash-safe?

       1. mysql的server层和存储引擎是分离， 导致记录在binlog的sql不知道是否已经写入磁盘
    ...
    9. mysql崩溃后恢复流程
       ...
       1. 读取redo log的checkpoint，并对日志进行重放（重放是指重新再内存中对数据进行修改）
       2. 检查redo log中哪些事务是完整并且处于prepare状态
       3. 根据xid对照binlog的事务是否完整（就是看事务有没有执行commit）
       4. 如果完整写redo log commit。 不完整就根据XID找到undo log进行回滚

    10. 一条update sql执行过程？

        1. 先记录undo log
        2. 修改数据页
        3. 记录redo log prepare
        4. 记录bin log
        5. 记录redo log  commit
    ...
    12. 索引
        ...
        9. 重建主键索引等价于重建整个表等价于 alter table T engine=InnoDB，不建议操作
        ...
        10. 索引失效场景？
            ...
            2. 使用了'LIKIE刘' ,>,<,!=
            3. 发生了类型转换
            4. 不同的字段使用了or：name='' or age=123 ot int,not exists， 部分字段不在索引中
            5. 字段可以为null：使用is null或者is not null可能导致索引失效
    ...
    13. buffer pool
     ...
     2. 空闲列表示例图（free链表）：![image-20211118110620724](image-20211118110620724.png)
...
12. 锁
       ...
       1. 锁的分类
                 1. 性能：乐观锁和悲观锁
                     2. 操作类型：读锁和写锁
                     3. 数据粒度：行锁、表级锁（表锁：lock tables aa read/write， MDL锁）、页锁
       2. 共享锁和排他锁
             1. 共享锁： lock in share mode
             2. 排它锁：for update

       3. MDL锁：增删改查加读锁，修改表结构加写锁
             1. 当对一个表执行crud时，对表加MDL读锁
             2. 当对表结构做变更操作时，加MDL写锁

       4. 示例图（ix：表级意向写锁）：![image-20211118131842460](image-20211118131842460.png)
       5. 间隙锁是什么？什么情况会加间隙锁？
       6. mysql的语句加上next lock，sql应该怎么写
       7. 行锁：
       8. mysql的语句具体在什么时候加锁？
...
13. 事务
       ...
       4. 怎么避免长事务对业务的影响？》
          1. 应用端
             1. set auto_commit=1, 检查框架是否会默认设置成0
             2. 去掉只读事务
             3. set_max_execution_time控制每个语句执行的最长时间
          2. 数据库端
             1. 监控information_schema.innodb_trx表，设置长事务阈值， 超过就报警或者kill
             2. 使用percona的pt-kill
             3. 业务功能测试阶段输出general_log
             4. set innodb_undo_tablespaces =2
...
22. 为什么会发送主从同步延迟？
   ...
   24. 一个6亿的表a，一个3亿的表b，通过外间tid关联，你如何最快的查询出满足条件的第50000到第50200中的这200条数据记录
...
18. redis
   ...
   20. 5种基本数据结构+bitmap、geo、hyperLoglog+redis module+redis stream
   ...
   25. redis的过期时间是unix时间戳, 某个key即使过期了，从节点也不会删除它，除非主节点发来了删除请求
   ...
   25. redis的文件事件和时间时间？
       ...
       3. 文件事件和时间事件是合作关系，服务器会轮流处理这两种请求
   ...
   29. 主从
       ...
       1. 全量同步：
          ...
          1. 具体流程
             ...
             12. ![image-20230522114109866](image-20230522114109866.png)
             12. runID：redis启动时生成的随机ID -1表示offset，全量复制就是-1
   ...
   37. redis的redlock？
        ...
        2. 流程：
           ...
           4. 加锁失败向所有节点发起释放锁请求
          3. redlock有什么问题？
        1. ![image-20211123151129134](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20211123151129134.png)
        2. 时间同步会导致时间跳跃，也有可能有问题：redis作者认为时间同步的误差较小
        3. 网络延迟和gc不止redlock有这个问题，其他组件也会有这个问题
   ...
   42. redis如何保证双写一致性？先更新缓存还是先更新数据库
   ...
   50. 如何解决redis的并发竞争key的问题？

          1. 分布式锁
          2. 消息队列

   51. 1亿个key，10万个key是已知前缀，怎么把它们全找出来？

          1. keys：会阻塞redis
          2. scan：一次取批，最后汇总。存在重复的概率，需要在客户端进行一次去重

   52. mysql有2000w数据，redis只存20w数据，怎么保证redis的数据就是热数据
   ...
   57. redis怎么删除大key（指某个key的value很大）？
